document.addEventListener('DOMContentLoaded', function () {
    ...
    function calculatePeriod(matrix) {
        const initialState = matrix.map(row => row.slice());
        let currentState = matrix.map(row => row.slice());
        let period = 0;

        do {
            period++;
            let newState = currentState.map(row => row.slice());
            for (let i = 0; i < currentState.length; i++) {
                for (let j = 0; j < currentState.length; j++) {
                    newState[i][j] = currentState[i].reduce((sum, val, k) => sum ^ (val & initialState[k][j]), 0);
                }
            }
            currentState = newState;
        } while (!initialState.every((row, i) => row.every((val, j) => val === currentState[i][j])));
    ...
    function calculateTheoreticalPeriod(matrixA, matrixB) {
        const periodA = calculatePeriod(matrixA);
        const periodB = calculatePeriod(matrixB);
        return periodA * periodB;
    }

    function calculateRealPeriod(sequenceA, sequenceB) {
        const periodicAcfA = calculatePeriodicACF(sequenceA);
        const periodicAcfB = calculatePeriodicACF(sequenceB);
        
        const maxLagA = periodicAcfA.findIndex((value, index) => index > 0 && value === 0);
        const maxLagB = periodicAcfB.findIndex((value, index) => index > 0 && value === 0);
        
        const realPeriodA = maxLagA > 0 ? maxLagA : sequenceA.length;
        const realPeriodB = maxLagB > 0 ? maxLagB : sequenceB.length;
        
        return realPeriodA * realPeriodB;
    }
    ...
    function calculate() {
        ...
        const sequenceA = generateSequence(degree);
        const sequenceB = generateSequence(degree2);
        const binarySequenceA = generateBinarySequence(degree);
        const binarySequenceB = generateBinarySequence(degree2);

        sequenceInput.value = sequenceA.join(', ');
        binarySequenceInput.value = binarySequenceA.join(', ');

        const polynomialA = calculatePolynomial(matrixA);
        const polynomialB = calculatePolynomial(matrixB);
        const periodA = calculatePeriod(matrixA);
        const periodB = calculatePeriod(matrixB);
        const theoreticalPeriod = calculateTheoreticalPeriod(matrixA, matrixB);
        const realPeriod = calculateRealPeriod(sequenceA, sequenceB);
        const hammingWeight = calculateHammingWeight(sequenceA);
        ...
        const periodicAcf = calculatePeriodicACF(sequenceA);
        plotACF(periodicAcf);
