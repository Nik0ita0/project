...
document.getElementById("calculateButton").addEventListener("click", function() {
    ...
    const generatorState = calculateGeneratorState(invertedMatrix, initialState);
...
function calculateGeneratorState(invertedMatrix, initialState) {
    let state = Array(invertedMatrix.length).fill(0);
    for (let i = 0; i < invertedMatrix.length; i++) {
        for (let j = 0; j < invertedMatrix.length; j++) {
            state[i] ^= invertedMatrix[i][j] * initialState[j];
        }
    }
    return state;
}
